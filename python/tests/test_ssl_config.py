...
from dify_plugin.core.utils.ssl import _create_ssl_context, _decode_base64
...
def test_decode_ca_cert_invalid_utf8_raises_error():
    ...
    with pytest.raises(ValueError, match="CA certificate must be valid UTF-8 PEM format"):
...
def test_ssl_patch_applies_config_from_env():
    ...
    with patch("dify_plugin.core.utils.ssl._config", mock_config_false):
        client = httpx.Client()
    ...
    with patch("dify_plugin.core.utils.ssl._config", mock_config_true):
        client = httpx.Client()
        assert client is not None
        client.close()
