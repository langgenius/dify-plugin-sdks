"""SMS Status Callback Event Handler."""
...
class SmsStatusEvent(Event):
    """Handle SMS status callback events."""

    def _on_event(self, request: Request, parameters: dict[str, Any], payload: dict[str, Any]) -> Variables:
        """
        Process SMS status webhook and apply filters.
        ...
        Returns:
            Variables containing the SMS status payload
        ...
        # Filter for errors only if configured
        if parameters.get("error_code_filter"):
        ...
        return Variables(
            variables={
                ...
                "date_sent": payload.get("DateSent", ""),
