...
class PullRequestAssignedTrigger(TriggerEvent):
    """
    GitHub Pull Request Pull Request Assigned Event Trigger

    This trigger handles GitHub pull request assigned events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request assigned event trigger
        ...
        # Verify this is a assigned action
        action = payload.get("action", "")
        if action != "assigned":
            # This trigger only handles assigned events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'assigned\'")
