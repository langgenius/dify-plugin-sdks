...
class PullRequestAutoMergeEnabledTrigger(TriggerEvent):
    """
    GitHub Pull Request Pull Request Auto Merge Enabled Event Trigger

    This trigger handles GitHub pull request auto_merge_enabled events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request auto_merge_enabled event trigger
        ...
        # Verify this is a auto_merge_enabled action
        action = payload.get("action", "")
        if action != "auto_merge_enabled":
            # This trigger only handles auto_merge_enabled events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'auto_merge_enabled\'")
