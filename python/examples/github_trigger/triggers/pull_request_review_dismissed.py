...
class PullRequestReviewDismissedTrigger(TriggerEvent):
    """
    GitHub Pull Request Review Pull Request Review Dismissed Event Trigger

    This trigger handles GitHub pull request review dismissed events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request review dismissed event trigger
        ...
        # Verify this is a dismissed action
        action = payload.get("action", "")
        if action != "dismissed":
            # This trigger only handles dismissed events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'dismissed\'")
