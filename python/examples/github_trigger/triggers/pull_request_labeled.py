...
class PullRequestLabeledTrigger(TriggerEvent):
    """
    GitHub Pull Request Pull Request Labeled Event Trigger

    This trigger handles GitHub pull request labeled events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request labeled event trigger
        ...
        # Verify this is a labeled action
        action = payload.get("action", "")
        if action != "labeled":
            # This trigger only handles labeled events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'labeled\'")
