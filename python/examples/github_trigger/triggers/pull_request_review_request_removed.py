...
class PullRequestReviewRequestRemovedTrigger(TriggerEvent):
    """
    GitHub Pull Request Pull Request Review Request Removed Event Trigger

    This trigger handles GitHub pull request review_request_removed events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request review_request_removed event trigger
        ...
        # Verify this is a review_request_removed action
        action = payload.get("action", "")
        if action != "review_request_removed":
            # This trigger only handles review_request_removed events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'review_request_removed\'")
