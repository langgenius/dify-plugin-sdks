...
class PullRequestConvertedToDraftTrigger(TriggerEvent):
    """
    GitHub Pull Request Converted to Draft Event Trigger

    This trigger handles GitHub pull request converted to draft events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request converted to draft event trigger
        ...
        # Verify this is a converted_to_draft action
        action = payload.get("action", "")
        if action != "converted_to_draft":
            # This trigger only handles converted_to_draft events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'converted_to_draft\'")
        ...
        variables = {
            "pull_request": {
                ...
                "draft": pull_request.get("draft", True),
                "head": {
