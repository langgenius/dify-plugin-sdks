...
class IssueEditedTrigger(TriggerEvent):
    """
    GitHub Issue Edited Event Trigger
    
    This trigger handles GitHub issue edited events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub issue edited event trigger
        ...
        if "title" in changes:
            changes_data["title"] = {
                "from": changes["title"].get("from", "")
            }
        if "body" in changes:
            changes_data["body"] = {
                "from": changes["body"].get("from", "")
            }
