...
class RepositoryPrivatizedTrigger(TriggerEvent):
    """
    GitHub Repository Repository Made Private Event Trigger

    This trigger handles GitHub repository privatized events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub repository privatized event trigger
        """
        ...
        # Verify this is a privatized action
        action = payload.get("action", "")
        if action != "privatized":
            # This trigger only handles privatized events
