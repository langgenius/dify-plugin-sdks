...
class PullRequestReviewRequestedTrigger(TriggerEvent):
    """
    GitHub Pull Request Pull Request Review Requested Event Trigger

    This trigger handles GitHub pull request review_requested events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request review_requested event trigger
        ...
        # Verify this is a review_requested action
        action = payload.get("action", "")
        if action != "review_requested":
            # This trigger only handles review_requested events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'review_requested\'")
