...
class PullRequestUnlockedTrigger(TriggerEvent):
    """
    GitHub Pull Request Pull Request Unlocked Event Trigger

    This trigger handles GitHub pull request unlocked events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub pull request unlocked event trigger
        ...
        # Verify this is a unlocked action
        action = payload.get("action", "")
        if action != "unlocked":
            # This trigger only handles unlocked events
            raise TriggerIgnoreEventError(f"Action \'{action}\' is not \'unlocked\'")
