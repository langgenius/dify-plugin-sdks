...
class RepositoryUnarchivedTrigger(TriggerEvent):
    """
    GitHub Repository Repository Unarchived Event Trigger

    This trigger handles GitHub repository unarchived events and extracts relevant
    information from the webhook payload to provide as variables to the workflow.
    """

    def _trigger(self, request: Request, parameters: Mapping[str, Any]) -> Event:
        """
        Handle GitHub repository unarchived event trigger
        """
        ...
        # Verify this is a unarchived action
        action = payload.get("action", "")
        if action != "unarchived":
            # This trigger only handles unarchived events
