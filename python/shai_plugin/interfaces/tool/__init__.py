...
from shai_plugin.entities import ParameterOption
...
from shai_plugin.entities.tool import ToolInvokeMessage, ToolParameter, ToolRuntime, ToolSelector
from shai_plugin.file.constants import SHAI_FILE_IDENTITY, SHAI_TOOL_SELECTOR_IDENTITY
from shai_plugin.file.entities import FileType
from shai_plugin.file.file import File
from shai_plugin.protocol.oauth import OAuthCredentials
...
class ToolLike(ABC, Generic[T]):
    ...
    def _convert_parameters(cls, tool_parameters: dict) -> dict:
        ...
        for parameter, value in tool_parameters.items():
            if isinstance(value, dict) and value.get("shai_model_identity") == SHAI_FILE_IDENTITY:
                ...
                isinstance(item, dict) and item.get("shai_model_identity") == SHAI_FILE_IDENTITY for item in value
            ):
            ...
            elif isinstance(value, dict) and value.get("shai_model_identity") == SHAI_TOOL_SELECTOR_IDENTITY:
                tool_parameters[parameter] = ToolSelector.model_validate(value)
            elif isinstance(value, list) and all(
                isinstance(item, dict) and item.get("shai_model_identity") == SHAI_TOOL_SELECTOR_IDENTITY
                for item in value
            ):
